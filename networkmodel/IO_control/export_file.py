...
def write_csv_file_noSpark(OUTPUT_PATH:str, df:pd.DataFrame, date:str, f_name:str, need_index:bool=False) -> bool:    
    # パス先がなければディレクトリの作成
    if not os.path.isdir(OUTPUT_PATH):
        os.makedirs(OUTPUT_PATH, exist_ok=True)
    # csv出力
    df.to_csv(OUTPUT_PATH+f'{date}_{f_name}.csv', index=need_index, header=True)
    return True

def write_csv_file_with_date_noSpark(OUTPUT_PATH:str, df:pd.DataFrame, date:str, f_name:str, need_index:bool=False) -> bool:
    folder_path = OUTPUT_PATH + f'year={date[0:4]}/month={date[0:7]}/date={date}/'
    # パス先がなければディレクトリの作成
    if not os.path.isdir(folder_path): 
        os.makedirs(folder_path, exist_ok=True)
    # csv出力
    df.to_csv(folder_path+f'{date}_{f_name}.csv', index=need_index, header=True)
    return True
